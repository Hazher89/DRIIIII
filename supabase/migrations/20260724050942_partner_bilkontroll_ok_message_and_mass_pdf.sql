-- Profesjonell OK-tekst ved bilkontroll uten avvik (SMS / push / e-post).
-- Inkluderer skiltnummer og positiv, profesjonell bekreftelse.
...
BEGIN
  ...
  ELSE
    v_sms := format(
      'MAVI Logistikk: Bil med %s hos %s har vært i kontroll. '
      || 'Alt ser bra ut — bra jobba!%s '
      || 'Rapport: https://driftpro.no',
      v_vehicle,
      v_partner_name,
      CASE
        WHEN v_next_date IS NOT NULL THEN ' Neste kontroll ' || v_next_date || '.'
        ELSE ''
      END
    );
    v_push_title := CASE
      WHEN v_plate IS NOT NULL THEN 'Alt ser bra ut — ' || v_plate
      ELSE 'Bilkontroll: alt ser bra ut'
    END;
    v_push_body := format(
      'Bil med %s hos %s har vært i kontroll. Alt ser bra ut — bra jobba!',
      v_vehicle, v_partner_name
    );
    v_owner_subject := format(
      'Bilkontroll OK — bra jobba (%s)',
      coalesce(v_plate, v_mavi, 'kjøretøy')
    );
  END IF;
  ...
  IF coalesce(p_send_email, false) THEN
    v_owner_body := format(
      ...
      CASE
        ...
        ELSE 'Bilen har vært i kontroll, og alt ser bra ut — bra jobba!' || E'\n'
      END,
    ...
    BEGIN
      v_owner_html := public.build_driftpro_email_html(
        CASE
          WHEN v_has_dev THEN 'Avvik etter bilkontroll'
          ELSE 'Bilkontroll OK — bra jobba'
        END,
        CASE
          ...
          ELSE format(
            'Bil med %s har vært i kontroll. Alt ser bra ut — bra jobba! Rapporten er tilgjengelig i DriftPro.',
            v_vehicle
          )
        END,
        jsonb_build_array(
